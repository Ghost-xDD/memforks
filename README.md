...
Persistent memory systems solve the _storage_ half: durable, encrypted, semantically recalled context. But persistence alone leaves memory as a **flat, linear append-log**, and that breaks down the moment agents do real work:
...
MemForks turns durable agent memory into a versioned commit graph: the same conceptual leap that Git made for code, applied to what AI agents learn and remember.

| Layer                                                                                           | Responsibility                                         |
| ----------------------------------------------------------------------------------------------- | ------------------------------------------------------ |
| ![](https://img.shields.io/badge/-Storage-2f6f6f?style=flat-square&labelColor=12241b)           | Encrypted blob storage and semantic recall             |
| ![](https://img.shields.io/badge/-Version%20Control-2f6f4f?style=flat-square&labelColor=12241b) | Immutable commit DAG, branch semantics, merge protocol |
| ![](https://img.shields.io/badge/-Verification-3a7bd5?style=flat-square&labelColor=12241b)      | Cryptographic anchoring, resolver voting, finality     |

MemForks records _when_ memories were created, _which branch_ they belong to, and _how_ conflicting memories get reconciled.

Three primitives:

1. **Forkable memory trees.** Branches are named pointers into a content-addressed commit DAG. Commits are encrypted blobs; forking is a pointer write, free and instant.
...
> MemForks versions what the agent _knows_, not what it _makes_. Artifact storage (datasets, reports, files an agent produces) is a sibling concern; commits carry `ArtifactRef` entries so attached files inherit the same provenance trail — and the CLI exposes this as `memfork commit --file <path>`.
...
| -------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Agent app builders** (LangGraph, Vercel AI SDK)                    | One-line adapter replaces the hand-rolled vector-DB memory layer and adds branching per user/session, A/B strategies, rollback, and per-fact provenance     |
...
That's it. No wallet, gas management, or dashboard. Restart Cursor and the agent recalls and commits memory across sessions, scoped to the current Git branch, with hash-chained commits and verifiable merges.
...
| ---------------------------- | --------------------------------------------------------- |
| Recall prior context         | Installed memory tools scoped to the active branch        |
| Record a learned fact        | `memfork commit --branch <b> --facts "…"`                  |
| Attach files to a commit     | `memfork commit --file report.md --file data.csv`          |
...
The installed MCP integration handles storage and recall as native tool calls. The `memfork` CLI handles versioning, branching, artifacts, and governed merges.
...
2. Provisions the account and delegate credentials
3. Registers the delegate
4. Creates the initial `MemoryTree`
5. Saves credentials to `~/.memfork/credentials.json`

### memfork install

`memfork install cursor` configures the memory MCP connection in `~/.cursor/mcp.json` and installs `.cursor/rules/memforks.mdc`, an always-on rule that teaches the agent when to recall context and create durable commits.

No browser login is required. Credentials flow directly from provisioning into the MCP configuration.
...
A developer tool for inspecting, debugging, and replaying agent memory: a live DAG explorer with a commit inspector and real-time event polling. Commits with attached artifacts show a 📎 badge in the timeline; the inspector panel renders each artifact with a direct download link. `memfork ui` opens it against your tree, or run it standalone:

<!-- Drop a screenshot or GIF at docs/assets/visualizer.png (record the DAG updating live from mainnet events). -->

![MemForks visualizer — live commit DAG](docs/assets/visualizer.png)
...
- Every commit carries parent IDs and content hashes, producing a verifiable history.
- Encrypted memory remains portable across machines and sessions.
- Artifacts are first-class. `memfork commit --file <path>` attaches reports, datasets, and logs; each commit records an `ArtifactRef` with its blob ID, SHA-256 digest, path, and size.

**Governed version control.**

- `MemoryTree` records branches, commit anchors, ownership, and delegated permissions.
- Jury merges collect signed attestations and enforce a k-of-n threshold before advancing a branch head.
- Resolver policies support deterministic, model-assisted, and multi-reviewer reconciliation.
- The visualizer follows protocol events for real-time DAG updates.

**Privacy by default.**

- Memory is encrypted at rest.
- Delegate credentials control read and write access.
- Sharing and revocation are explicit, auditable operations.

### Gas sponsorship

The two-command quickstart works because a sponsorship service absorbs transaction costs on the user's behalf—no wallet funding, faucet, or dashboard. It runs as a small stateless service in [`services/sponsor`](services/sponsor).

Onboarding flows through two endpoints:

- **`POST /drip`** — provisions the small bootstrap balance required during `memfork init --quick`. It is rate-limited to one request per IP per day and skipped when the address is already funded.
...
Then fund the configured storage and transaction accounts. Run `memfork doctor` to verify both balances. See [`docs/architecture/artifacts.md`](docs/architecture/artifacts.md) for the full design.
...
docs/
  ...
  architecture.md       Stack diagram, authentication chain, and data flows
...
| **Verifiable provenance**     | ✅ ledger-backed       | ~ local          | ~ local           | ✗             | ✗                   |
...
| **Recall benchmarks**         | ~ backend-dependent    | ~ in-repo        | ✅ 98.9%          | ✅ 94.8       | ✅ 75% (own)        |
...
- **Protocol contracts** (`memforks::tree`, `memforks::acl`, `memforks::resolver`): deployed on mainnet. Branch creation, merge proposals, k-of-n attestation collection, and finalization are enforced on-chain.
...
MemForks is the shared remote for agent memory: a substrate other agent systems can plug into so that what an agent learns is durable, portable, governable, and verifiable by default.

### What's next

Built during the hackathon; here's where it goes from here.

- **Self-hosted memory gateway.** Give teams a fully self-hostable path for storage, recall, and credential management.
- **Per-branch cryptographic isolation.** Encrypt and authorize every branch independently rather than relying only on logical namespace boundaries.
...
| [docs/architecture.md](./docs/architecture.md)                     | Stack diagram, authentication chain, protocol components, and data flows        |
